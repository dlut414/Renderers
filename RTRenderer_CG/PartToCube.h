...
public:
    ...
    template <typename T>
    const T inpT(const Point& p, const T* const data) const
    {
        T _ret = T();
        ...
        _ret += 0.125f * (1 - _v.x) * (1 - _v.y) * (1 - _v.z) * data[_id[0]];
        _ret += 0.125f * (1 + _v.x) * (1 - _v.y) * (1 - _v.z) * data[_id[1]];
        _ret += 0.125f * (1 + _v.x) * (1 + _v.y) * (1 - _v.z) * data[_id[2]];
        _ret += 0.125f * (1 - _v.x) * (1 + _v.y) * (1 - _v.z) * data[_id[3]];
        _ret += 0.125f * (1 - _v.x) * (1 - _v.y) * (1 + _v.z) * data[_id[4]];
        _ret += 0.125f * (1 + _v.x) * (1 - _v.y) * (1 + _v.z) * data[_id[5]];
        _ret += 0.125f * (1 + _v.x) * (1 + _v.y) * (1 + _v.z) * data[_id[6]];
        _ret += 0.125f * (1 - _v.x) * (1 + _v.y) * (1 + _v.z) * data[_id[7]];
...
private:
    ...
    void buildMesh()
    {
        ...
        for(unsigned k=0; k<i_vz; k++)
        {
            for(unsigned j=0; j<i_vy; j++)
            {
                for(unsigned i=0; i<i_vx; i++)
                {
                    ...
                    velv.       push_back( Vector(0.f, 0.f, 0.f) );
                }
            }
        }
        ...
        for(unsigned i=0; i<velv.size(); i++)
        {
            Point       _verti  = vert[i];
            unsigned    _vox    = offset_voxel(_verti);
            ...
            for(unsigned nb = 0; nb < PartTo_::voxObj->neiList[_vox].size(); nb++)
            {
                ...
                for(unsigned j = 0; j < PartTo_::voxObj->eleList[_nbid].size(); j++)
                {
                    ...
                    Point __partj   =   PartTo_::part[_partjId];
                    float __dr      =   sqrt( (__partj - _verti).mag2() );
                    velv[i]         +=  PartTo_::vel[_partjId] * weight_cubic(voxWidth, __dr);
                }
            }
            /*
            for(unsigned j=0; j<part.size(); j++)
