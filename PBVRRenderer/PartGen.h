    ...
    void genPart_MP(const PartToPart* const obj)
    {
        ...
        for(unsigned level=0; level<REPEAT_LEVEL; level++)
        {
            ...
            for(unsigned i=0; i<(obj->part.size()); i++)
            {
                ...
                float       __color1        = obj->inpVel(__pt1) / obj->maxVel;
                float       __dens1         = /*rho( obj->inpAlpha(__pt1)/obj->maxAlpha );*/rho( __color1 );
                float       __num           = /*rho(obj->alpha[i]/obj->maxAlpha) * (obj->capacity() / (8.f * pow(radius, 3)));*/rho(obj->vel[i].mag()/obj->maxVel) * (obj->capacity() / (8.f * pow(radius, 3)));
                ...
                for(unsigned n=0; n<__partInCube; n++)
                {
                    ...
                    float __color2  = obj->inpVel(__pt2) / obj->maxVel;
                    float __alpha2  = /*obj->inpAlpha(__pt2)/obj->maxAlpha;*/__color2;
