...
GLuint      vertexBufferPos_tri;
GLuint      vertexBufferNorm_tri;
GLuint      vertexBufferPos[REPEAT_LEVEL];
GLuint      vertexBufferNorm[REPEAT_LEVEL];
GLuint      vertexBufferColor[REPEAT_LEVEL];
...
Point*      r3_vertex_pos_data_tri;
Vector*     r3_vertex_norm_data_tri;
GLuint      dataSize_tri;
GLuint      numEdge_tri;

Point*      r3_vertex_pos_data;
Vector*     r3_vertex_norm_data;
float*      r_vertex_color_data;
GLuint      dataSize_r3;
GLuint      dataSize_r;
GLuint      numParticle;
...
GLfloat     f_pointRadius     = 1.f;
GLfloat     f_pointScale      = 1000.f;
...
void myDisplay();
GLuint LoadShader(const GLchar*, const GLchar*);
...
void InitOBJ()
{
    ...
    std::cout << (*data_obj << str) << std::endl;
    std::cout << (*tria_obj << str) << std::endl;

    data_obj    ->  build();
    tria_obj    ->  build();
    part_obj    ->  genPart_MP(data_obj);

    std::cout << "# of vertex: " << data_obj->part.size() << std::endl;
    std::cout << "# of particle: " << part_obj->numPart << std::endl;
    std::cout << "boundary1: " << data_obj->box.pMin.x << " " << data_obj->box.pMin.y << " " << data_obj->box.pMin.z << " " << std::endl;
    std::cout << "boundary2: " << data_obj->box.pMax.x << " " << data_obj->box.pMax.y << " " << data_obj->box.pMax.z << " " << std::endl;

    g_InitialCameraPosition = glm::vec3((data_obj->box.pCenter()).x, (data_obj->box.pCenter()).z, 0.02f);

    numParticle     = part_obj->numPart / REPEAT_LEVEL;
    dataSize_r3     = numParticle * sizeof(Point);
    dataSize_r      = numParticle * sizeof(float);

    r3_vertex_pos_data  = part_obj ->  part.  data();
    r3_vertex_norm_data = part_obj ->  normal.data();
    r_vertex_color_data = part_obj ->  color. data();

    numEdge_tri     =   tria_obj    ->  tri.size();
    dataSize_tri    =   numEdge_tri * sizeof(Point);

    r3_vertex_pos_data_tri  = tria_obj  ->  tri.    data();
    r3_vertex_norm_data_tri = tria_obj  ->  triNorm.data();
}
...
void InitVBO()
{
    glGenBuffers(1, &vertexBufferPos_tri);
    glBindBuffer(GL_ARRAY_BUFFER, vertexBufferPos_tri);

    glGenBuffers(1, &vertexBufferNorm_tri);
    glBindBuffer(GL_ARRAY_BUFFER, vertexBufferNorm_tri);
    ...
    for(int i=0; i<REPEAT_LEVEL; i++)
    {
        ...
        glGenBuffers(1, &vertexBufferColor[i]);
        glBindBuffer(GL_ARRAY_BUFFER, vertexBufferColor[i]);
    }
}
...
void Init(int argc, char** argv)
{
    ...
    programID2 = LoadShader("./shader2/vertex.glsl", "./shader2/fragment.glsl");
    ...
    matrixID[2] = glGetUniformLocation (programID2, "projectionMat");
    matrixID[3] = glGetUniformLocation (programID2, "viewModelMat");
...
void DrawBuffer_screen(int level)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, dataSize_r3, &r3_vertex_pos_data[_offset], GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

    glBindBuffer(GL_ARRAY_BUFFER, vertexBufferNorm[level]);
    glBufferData(GL_ARRAY_BUFFER, dataSize_r3, &r3_vertex_norm_data[_offset], GL_STATIC_DRAW);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

    glBindBuffer(GL_ARRAY_BUFFER, vertexBufferColor[level]);
    glBufferData(GL_ARRAY_BUFFER, dataSize_r, &r_vertex_color_data[_offset], GL_STATIC_DRAW);
    ...
    ///send data
    glUniformMatrix4fv(matrixID[2], 1, GL_FALSE, &g_projectionMat[0][0]);
    glUniformMatrix4fv(matrixID[3], 1, GL_FALSE, &g_viewModelMat[0][0]);

    glBindBuffer(GL_ARRAY_BUFFER, vertexBufferPos_tri);
    glBufferData(GL_ARRAY_BUFFER, dataSize_tri, r3_vertex_pos_data_tri, GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

    glBindBuffer(GL_ARRAY_BUFFER, vertexBufferNorm_tri);
    glBufferData(GL_ARRAY_BUFFER, dataSize_tri, r3_vertex_norm_data_tri, GL_STATIC_DRAW);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);
    ...
    glDrawArrays(GL_TRIANGLES, 0, numEdge_tri);
...
void Final()
{
    ...
    glDeleteBuffers(REPEAT_LEVEL, vertexBufferColor);
}
...
void myMotion(int x, int y)
{
    ...
    switch(i_mouseButton)
    {
        ...
        case GLUT_MIDDLE_BUTTON:
        {
            ...
            g_Scale     =  glm::max( g_Scale, glm::vec3(0.f, 0.f, 0.f) );
            break;
        }
    }
    glutPostRedisplay();
...
GLuint LoadShader(const GLchar* vertex_file_path, const GLchar* fragment_file_path)
{
