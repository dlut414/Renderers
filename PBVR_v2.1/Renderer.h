...
GLuint      dataSize_vector;
GLuint      dataSize_scalar;
GLuint      numParticle;
...
GLfloat     f_pointRadius     = 0.0005f;
GLfloat     f_pointScale      = 100.0f;
...
void InitOBJ()
{
    ...
    std::cout << (*cube_obj << str) << std::endl;

    cube_obj    ->  build();
    part_obj    ->  genPart_MP(cube_obj->obj);

    std::cout << "# of vertex: " << cube_obj->obj->vert.size() << std::endl;
    std::cout << "# of particle: " << part_obj->numPart << std::endl;
    std::cout << "boundary1: " << cube_obj->obj->box.pMin.x << " " << cube_obj->obj->box.pMin.y << " " << cube_obj->obj->box.pMin.z << " " << std::endl;
    std::cout << "boundary2: " << cube_obj->obj->box.pMax.x << " " << cube_obj->obj->box.pMax.y << " " << cube_obj->obj->box.pMax.z << " " << std::endl;

    g_InitialCameraPosition = glm::vec3((cube_obj->obj->box.pCenter()).x, (cube_obj->obj->box.pCenter()).z, 0.05f);

    numParticle     = part_obj->numPart / REPEAT_LEVEL;
    dataSize_vector = numParticle * sizeof(Point);
    dataSize_scalar = numParticle * sizeof(float);
...
void DrawBuffer_screen(int level)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, dataSize_vector, &r3_vertex_pos_data[_offset], GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

    glBindBuffer(GL_ARRAY_BUFFER, vertexBufferNorm[level]);
    glBufferData(GL_ARRAY_BUFFER, dataSize_vector, &r3_vertex_norm_data[_offset], GL_STATIC_DRAW);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

    glBindBuffer(GL_ARRAY_BUFFER, vertexBufferColor[level]);
    glBufferData(GL_ARRAY_BUFFER, dataSize_scalar, &r_vertex_color_data[_offset], GL_STATIC_DRAW);
