...
{
public:
    PartGen(const float& r = 0.f, const float& t = 1.f, const float& beta = 1.f, const unsigned& s = 0)
        : radius(r), dt(t), isoValue(beta), rp(s)
    {
        allocPart();
    }
    ~PartGen()
            ...
            for(unsigned k=0; k<(obj->i_cz); k++)
            {
                for(unsigned j=0; j<(obj->i_cy); j++)
                {
                    for(unsigned i=0; i<(obj->i_cx); i++)
                    {
                        ...
                        float       __color1        = obj->inpVel(__pt1).mag();
                        float       __dens1         = rho(obj->inpAlpha(__pt1));
                        float       __num           = rho(obj->inpAlpha(__box.pCenter())) * (obj->capacity()) / (8.f * pow(radius, 3));
                        ...
                        for(unsigned n=0; n<__partInCube; n++)
                        {
                            ...
                            float __color2  = obj->inpVel(__pt2).mag();
                            float __alpha2  = obj->inpAlpha(__pt2);
                            float __dens2   = rho(__alpha2);
                            float __w       = __dens2 / __dens1;

                            if(__alpha2 < isoValue) continue;
